...
def mnist_inn_one(mask_size=[28, 28]):
    ...
    r2 = fr.Node([r1.out0], re.reshape_layer, {'target_dim': (img_dims[0]*img_dims[1]*img_dims[2],)}, name='r2')
...
def celeba_inn_small(mask_size=[156, 128]):
    ...
    r2 = fr.Node([conv13.out0], re.reshape_layer, {'target_dim': (img_dims[0]*img_dims[1]*img_dims[2],)}, name='r2')

    fc = fr.Node([r2.out0], la.rev_multiplicative_layer,
                 {'F_class': fu.F_small_connected, 'F_args': {'internal_size': 5000}, 'clamp': 1}, name='fc')

    r3 = fr.Node([fc.out0], re.reshape_layer, {'target_dim': (12, 78, 64)}, name='r3')
